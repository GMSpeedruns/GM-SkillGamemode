...
function SWEP:FireExtraBullets()
	if self:Var( "GetPrivate", "Type", false ) == true and self.ShootNext and self.NextShoot < CurTime() and self.ShotsLeft > 0 then
		self:GlockShoot( false )
	end
end

function SWEP:GlockShoot( showanim )
	if self:Var( "GetPrivate", "Type", false ) == true then self.ShootNext = false end
	...
	if self:Var( "GetPrivate", "Type", false ) == true and self.ShotsLeft > 0 and not self.ShootNext then
		self.ShootNext = true
