...
function SWEP:Reload()
	...
	if self:Var( "GetPrivate", "Reloading", false ) then return end
	if self.Weapon:Clip1() < self.Primary.ClipSize and self.Owner:GetAmmoCount( self.Primary.Ammo ) > 0 then
		self:Var( "SetPrivate", "Reloading", true, self.Owner )
...
function SWEP:Think()
	if self:Var( "GetPrivate", "Reloading", false ) then
		if (self.Weapon.ReloadTimer or 0) < CurTime() then
			if self.Weapon:Clip1() >= self.Primary.ClipSize or self.Owner:GetAmmoCount( self.Primary.Ammo ) <= 0 then
				return self:Var( "SetPrivate", "Reloading", false, self.Owner )
			end
